...
try:
    ...
    raise RuntimeError(
        "Missing lightgbm package. Install it with: pip install lightgbm"
    ) from exc
...
def make_classifier(random_state: int = 42) -> LGBMClassifier:
...
def class_index_map(model: LGBMClassifier) -> Dict[int, int]:
...
def export_to_onnx(model: LGBMClassifier, output_path: Path) -> None:
...
def write_run_in_mt5(
    ...
    txt = f"""MODEL: LightGBMClassifier (3 classes)
SYMBOL: {args.symbol}
TIMEFRAME: {args.timeframe}
...
STEPS TO RUN IN MT5:
1. Copy the ml_strategy_classifier_lightgbm.onnx file next to the EA file .mq5.
2. Recompile the EA in MetaEditor.
3. Run the Strategy Tester ONLY on the TEST UTC window above.
4. For additional trend/ATR filters, start from the settings that performed best in previous tests.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a LightGBM model for MT5 and export to ONNX.")
    ...
    p.add_argument("--output-dir", default="output_lightgbm", help="Output directory")
    p.add_argument("--horizon-bars", type=int, default=8, help="Target horizon in closed bars")
    p.add_argument("--train-ratio", type=float, default=0.70, help="Chronological train ratio")
    p.add_argument("--label-quantile", type=float, default=0.67, help="Quantile of abs(fwd_ret_h) for labeling")
    p.add_argument("--prob-quantile", type=float, default=0.80, help="Quantile of entry probabilities on train")
    p.add_argument("--margin-quantile", type=float, default=0.65, help="Quantile of probability gaps on train")
...
def main() -> None:
    ...
    print(f"\nLabeling barrier for abs(fwd_ret_h): {barrier:.8f}")
    ...
    onnx_path = output_dir / "ml_strategy_classifier_lightgbm.onnx"
